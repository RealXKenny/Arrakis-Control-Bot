...
import { DuneConsoleClient } from "../../../src/infrastructure/http/dune-console/DuneConsoleClient";
import { DiscordAdapterClient } from "../../../src/infrastructure/http/discord-adapter/DiscordAdapterClient";
import { ENDPOINT_CATALOG, loadEndpointCatalog } from "../../../src/infrastructure/http/dune-console/endpointCatalog";
...
describe("external API clients", () => {
  it("ships the complete Dune endpoint catalog without external reference files", () => {
    ...
    expect(ENDPOINT_CATALOG).toContainEqual({
      ...
      description: "List active CHOAM exchange sell orders aggregated by item + grade (paginated): lowest price, total stock, listing count",
    });
  });
  ...
  it("downloads bounded binary Console responses with bearer authentication", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response("archive-data", {
      headers: {
        "content-type": "application/octet-stream",
        "content-disposition": 'attachment; filename="dune.backup"',
      },
    }));
    vi.stubGlobal("fetch", fetchMock);

    const client = new DuneConsoleClient("https://console.example.com", "scoped-key");
    const response = await client.requestBinary("GET", "/api/backups/dune.backup/download", 1024);
    const request = fetchMock.mock.calls[0][1] as RequestInit;
    const headers = new Headers(request.headers);

    expect(response.data.toString()).toBe("archive-data");
    expect(response.filename).toBe("dune.backup");
    expect(headers.get("authorization")).toBe("Bearer scoped-key");
  });

  it("rejects binary Console responses larger than the configured bound", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response("oversized", { headers: { "content-length": "9" } })));
    const client = new DuneConsoleClient("https://console.example.com", "scoped-key");
    await expect(client.requestBinary("GET", "/api/backups/large/download", 8)).rejects.toThrow("exceeds");
  });
