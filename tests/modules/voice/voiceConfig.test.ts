...
it("leaves saved setup authoritative when environment IDs are blank", () => {
  ...
  expect(loadVoiceSetup({ GUILD_ID: " " })).toBeUndefined();
});
...
it("requires a complete set of valid IDs and persistent storage", () => {
  ...
  expect(() => loadVoiceSetup({ ...env, GUILD_ID: "invalid" })).toThrow("GUILD_ID");
  expect(() => loadVoiceSetup({ ...env, DATABASE_URL: "" })).toThrow("DATABASE_URL");
  expect(loadVoiceSetup(env)).toEqual({ guildId: env.GUILD_ID, joinChannelId: env.VOICE_JOIN_CHANNEL_ID, categoryId: env.VOICE_CATEGORY_ID, panelChannelId: env.VOICE_PANEL_CHANNEL_ID });
});

it("accepts the legacy voice-specific guild ID during migration", () => {
  expect(loadVoiceSetup({ ...env, GUILD_ID: undefined, VOICE_GUILD_ID: env.GUILD_ID })?.guildId).toBe(env.GUILD_ID);
});
