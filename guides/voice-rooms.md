...
Voice actions are grouped under `/voice` (for example `/voice setup`, `/voice rename`, and `/voice kick`). Retired `/voice-*` commands are removed when command registration synchronizes after restart. Public panel buttons retain their existing behavior and owner checks.
