...
The bot automatically publishes a public Music Lounge control panel in `MUSIC_REQUEST_CHANNEL_ID`. No separate panel setting is needed. It uses the voice panel's banner style and offers Request Song, Now Playing, View Queue, Pause, Resume, Skip, Volume, Clear Queue, and Stop Playback. Requests and volume open private forms; stop and clear ask for confirmation. Everyone can inspect playback and the queue. Listeners can add songs; only the current song's requester can change playback, checked again when forms and confirmations are submitted.
