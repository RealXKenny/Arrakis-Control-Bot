...
async function loadGuildMemberships(guildRows: GuildRow[]): Promise<GuildMembershipResult[]> {
  const results = await Promise.all(
    guildRows.map(async (guildRow) => {
      ...
      try {
        const membersResponse = await container.client.duneApi.call("GET", "/api/guilds/{guildId}/members", {
          routeParams: {
            guildId,
          },
        });
  ...
  return results.filter(filterNullish);
}

async function loadProfileData(playerId: string | number): Promise<ProfileData> {
  const endpoints = ["currency", "solaris-coin", "factions", "intel", "specs", "progression", "vitals"] as const;

  const responses = await Promise.all(
    endpoints.map(async (endpoint) => {
      try {
        const response = await container.client.duneApi.call("GET", `/api/players/{playerId}/${endpoint}`, {
          routeParams: {
            playerId,
          },
        });
...
const command = {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const data = await loadProfileData(playerId);
...
class ProfileCommand extends Command {
  public constructor(context: Command.LoaderContext, options: Command.Options) {
    super(context, { ...options, name: "profile", description: "Show your linked Dune player profile.", preconditions: ["InteractionRateLimit"] });
  }
