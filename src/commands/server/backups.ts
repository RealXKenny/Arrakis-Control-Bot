...
const asObject = (value: unknown): ApiObject | null => (isObject(value) ? (value as ApiObject) : null);
...
class BackupsCommand extends Command {
  public constructor(context: Command.LoaderContext, options: Command.Options) {
    super(context, { ...options, name: "backups", description: "Show available Dune server backups and auto-backup status.", preconditions: ["InteractionRateLimit"] });
  }
...
function getTimestampFromUnknown(value: unknown): number {
  ...
  if (isObject(value)) {
    return getTimestamp(getFirstDefined(value as ApiObject, ["timestamp", "createdAt", "created_at", "date"]) as TimestampValue);
  }
...
function getErrorDetails(error: unknown): {
  ...
  const errorRecord = error as ApiObject;
  const details = asObject(errorRecord.details);

  const message = typeof errorRecord.message === "string" ? errorRecord.message : typeof details?.error === "string" ? details.error : "Unknown error";

  const statusValue = errorRecord.status ?? details?.status;
