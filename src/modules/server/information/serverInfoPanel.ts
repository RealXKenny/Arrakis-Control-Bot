...
async function ensureServerInfoPanel(client: Client, channelId?: string | null): Promise<void> {
  ...
  const [existingPrimary, existingContinuation] = await Promise.all([
    findPanelMessage(channel, client.user.id, PANEL_MARKER),
    findPanelMessage(channel, client.user.id, PANEL_CONTINUATION_MARKER),
  ]);
