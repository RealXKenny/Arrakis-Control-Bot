...
import { ChannelType, EmbedBuilder, escapeMarkdown, PermissionFlagsBits, type Client, type Guild, type Message, type VoiceState } from "discord.js";
...
export class MusicService {
  ...
  public requireRequester(userId: string): void {
    ...
    if (this.current.requester !== userId) throw new MusicUserError("Only the person who requested the current song can change playback or clear the queue.");
  }
  ...
  private recover(): void {
    ...
    if (!this.panel.ready) void this.panel.publish().catch(() => {
